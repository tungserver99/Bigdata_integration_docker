...
if [[ -n "${LIVY_SESSION_TIMEOUT}" ]]; then
  echo "livy.server.session.timeout=${LIVY_SESSION_TIMEOUT}" >> "${LIVY_CONF_DIR}/livy.conf"
fi
if [[ -n "${LIVY_TIMEOUT_CHECK}" ]]; then
  echo "livy.server.session.timeout-check=${LIVY_TIMEOUT_CHECK}" >> "${LIVY_CONF_DIR}/livy.conf"
fi
